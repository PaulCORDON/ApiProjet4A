...
import fr.ensim.projet4a.model.Calculs;
...
public class ExampleRestController {
	...
	/**
	 * Récupère tous les eleves qui ont le paramètre param d'apliquer dans
	 * l'exercice 1 de calcul
	 */
	...
	public ResponseEntity<Eleve> postEleve(@RequestBody @Valid Eleve eleve) {
		...
		URI location = ServletUriComponentsBuilder.fromCurrentRequest().path("/{nomprenom,dateDeNaissance,classeName}")
				.buildAndExpand(eleve).toUri();
	...
	public void updateEleve(@PathVariable @NotNull String nomPrenom, @PathVariable @NotNull String nom,
			@RequestBody @Valid Eleve eleve) {
		ServletUriComponentsBuilder.fromCurrentRequest().path("/{nomprenom,dateDeNaissance,classeName}")
				.buildAndExpand(eleve).toUri();
	...
	public ResponseEntity<ParamEl1> postParamEl1(@RequestBody @Valid ParamEl1 param) {
		...
		URI location = ServletUriComponentsBuilder.fromCurrentRequest().path(
				"/{nom,enonceDisparait,multipleApparution,nbApparition,tempsApparution,nbEnonce,nbAparitionSimultanee}")
				.buildAndExpand(param).toUri();
	...
	public ResponseEntity<ParamEm1> postParamEm1(@RequestBody @Valid ParamEm1 param) {
		...
		// URI de localisation de la ressource
		URI location = ServletUriComponentsBuilder.fromCurrentRequest().path(
				"/{nom,nbBornes, nbQuestions,disparition,tempsRestantApparant, ordreApparition, borneSelectionnable, borneEqualsOp, valMax, frise, tempsRep, pairOnly, operateur1, operateur2, operateur3, operateur4}")
				.buildAndExpand(param).toUri();
	...
	public ResponseEntity<ParamEm2> postParamEm2(@RequestBody @Valid ParamEm2 param) {
		...
		// URI de localisation de la ressource
		URI location = ServletUriComponentsBuilder.fromCurrentRequest().path(
				"/{nom, typeRep, nbCalcul, valMaxOperande,nombrePair, nombreImpair, typeNombre, repDeuxBornes, repQuatreBornes, repPaveNum, tempsRep, pairOnly, operateur1, operateur2, operateur3, operateur4, calcChaine}")
				.buildAndExpand(param).toUri();
	...
	public void appliqueParamEm2(@PathVariable @NotNull String nomPrenom, @PathVariable @NotNull String nom,
			@PathVariable @NotNull String nomParam) {
	...
	public void appliqueParamEm1(@PathVariable @NotNull String nomPrenom, @PathVariable @NotNull String nom,
			@PathVariable @NotNull String nomParam) {
	...
	public void appliqueParamEl1(@PathVariable @NotNull String nomPrenom, @PathVariable @NotNull String nom,
			@PathVariable @NotNull String nomParam) {
	...
	public ArrayList<SousCompetence> getSousCompEleve(@PathVariable @NotNull String nomprenom,
			@PathVariable @NotNull String nom) {
	...
	/**
	 * Applique un enoncé à un paramètrage d'exercice 1 de lecture en désapliquant
	 * les anciennes énoncé du paramétrage
	 */
	@PostMapping("/paramEl1/{nomParam}/enonce")
	public void appliqueEnonceToParamEl1SupprHistorique(@PathVariable @NotNull String nomParam,
			@RequestBody @Valid Enonce enonce) {
	...
	public ArrayList<Exo1Math> getHistoriqueExercices1MEleve(@PathVariable @NotNull String nomprenom,
			@PathVariable @NotNull String nom) {
	...
	public ArrayList<Exo2Math> getHistoriqueExercices2MEleve(@PathVariable @NotNull String nomprenom,
			@PathVariable @NotNull String nom) {
	...
	public ArrayList<Exo1Lecture> getHistoriqueExercices1LEleve(@PathVariable @NotNull String nomprenom,
			@PathVariable @NotNull String nom) {
		return DBService.getHistoriqueEl1EleveFromBD(nomprenom, nom);
	}

	/**
	 * Ajoute un éxercice 1 de calcul que vient de faire un élève à la base de
	 * donnée
	 * 
	 * @param idParam
	 * @param idEleve
	...
	public void addEm1ToDB(@PathVariable @NotNull String nom, @PathVariable @NotNull int idEleve,
			@PathVariable @NotNull String res, @PathVariable @NotNull Timestamp date, @RequestBody @Valid Calculs c) {

		ServletUriComponentsBuilder.fromCurrentRequest().path("/{tabCalc}").buildAndExpand(c).toUri();
		DBService.addEm1ToDB(nom, idEleve, res, date, c.getTabCalc());
	}

	/**
	 * Ajoute un éxercice 2 de calcul que vient de faire un élève à la base de
	 * donnée
	 * 
	 * @param idParam
	 * @param idEleve
	...
	public void addEm2ToDB(@PathVariable @NotNull String nom, @PathVariable @NotNull int idEleve,
			@PathVariable @NotNull String res, @PathVariable @NotNull Timestamp date, @RequestBody @Valid Calculs c) {
		ServletUriComponentsBuilder.fromCurrentRequest().path("/{tabCalc}").buildAndExpand(c).toUri();

		DBService.addEm2ToDB(nom, idEleve, res, date, c.getTabCalc());
	}

	/**
	 * Ajoute un éxercice 1 de lecture que vient de faire un élève à la base de
	 * donnée
	 * 
	 * @param idParam
	 * @param idEleve
	...
	public void addEl1ToDB(@PathVariable @NotNull String nom, @PathVariable @NotNull int idEnonce,
			@PathVariable @NotNull int idEleve, @PathVariable @NotNull String res,
			@PathVariable @NotNull Timestamp date) {
